...
RSpec.describe RentsController, type: :controller do
  ...
  describe 'DELETE #destroy' do
    context 'when current user has admin rights' do
      it 'deletes the rent record' do
        user = FactoryGirl.create :user, :confirmed, :admin
        token = Sessions::Create.for credential: user.username, password: user.password

        rent_to_destroy = FactoryGirl.create :rent

        request.headers[:HTTP_AUTH_TOKEN] = token
        expect{ delete :destroy, params: { id: rent_to_destroy.id }}.to change(Rent, :count).to(0)
        
        expect(response).to be_success
      end
    end
    context 'when the user is an average user' do
      context 'and is trying to delete a rent he owns' do
        it 'does not delete the rent and returns forbidden' do
          user = FactoryGirl.create :user, :confirmed
          token = Sessions::Create.for credential: user.username, password: user.password
          ...
          request.headers[:HTTP_AUTH_TOKEN] = token
          delete :destroy, params: { id: rent_to_destroy.id }

          expect(response).to be_forbidden
        end
      end
      context 'and is trying to delete other user rent' do
        it 'does not delete the rent and returns forbidden' do
          user = FactoryGirl.create :user, :confirmed
          token = Sessions::Create.for credential: user.username, password: user.password
    ...
    context 'and the id is not from an actual rent' do
      it 'returns not found' do
          user = FactoryGirl.create :user, :confirmed
          token = Sessions::Create.for credential: user.username, password: user.password
